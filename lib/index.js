...
/**
 * The ECMAScript version suffixes used for the `typescript-*` configs (and
 * their `flat/typescript-*` counterparts). `base` disables every rule; each
 * numeric year turns on the rules for syntax introduced in that edition and
 * extends the previous year's config.
 * @typedef {"base" | 2016 | 2018 | 2019 | 2020 | 2021 | 2022 | 2023 | 2024 | 2025} EscompatTypeScriptConfigVersion
 */

/**
 * A flat config that is guaranteed to carry a `rules` block (unlike
 * `Linter.Config`, where `rules` is optional), so callers can do
 * `Object.keys(config.rules)` without a null check.
 * @typedef {import('eslint').Linter.Config & {
 *   rules: NonNullable<import('eslint').Linter.Config["rules"]>
 * }} EscompatFlatConfig
 */

/**
 * An eslintrc (legacy) config that is guaranteed to carry a `rules` block
 * (unlike `Linter.LegacyConfig`, where `rules` is optional).
 * @typedef {import('eslint').Linter.LegacyConfig & {
 *   rules: NonNullable<import('eslint').Linter.LegacyConfig["rules"]>
 * }} EscompatLegacyConfig
 */

/**
 * The concrete `configs` map exposed on the plugin's default export. Eslintrc
 * (legacy) configs are plain objects; flat configs are either a single config
 * object (`flat/recommended`) or an array of them (`flat/typescript*`). Every
 * entry except `typescript` (which only re-`extends` `typescript-2025`) carries
 * a populated `rules` block.
 * The trailing index signature keeps the map indexable by a computed string
 * (e.g. when iterating `Object.keys(configs)`); known keys still resolve to
 * their specific type.
 * @typedef {(
 *   {
 *     recommended: EscompatLegacyConfig,
 *     "flat/recommended": EscompatFlatConfig,
 *     typescript: import('eslint').Linter.LegacyConfig,
 *     "flat/typescript": EscompatFlatConfig[],
 *   }
 *   & Record<`typescript-${EscompatTypeScriptConfigVersion}`, EscompatLegacyConfig>
 *   & Record<`flat/typescript-${EscompatTypeScriptConfigVersion}`, EscompatFlatConfig[]>
 *   & Record<string, EscompatLegacyConfig | EscompatFlatConfig | import('eslint').Linter.LegacyConfig | EscompatFlatConfig[]>
 * )} EscompatConfigs
 */

/**
 * The shape of the plugin's default export: a fully populated `rules` map plus
 * the specific {@link EscompatConfigs} shape.
 * @typedef {Required<Pick<import('eslint').ESLint.Plugin, "rules">> & {
 *   configs: EscompatConfigs
 * }} EscompatPlugin
 */
...
createRule(
  "no-private-in",
  "edge < 91, safari < 15, firefox < 90, chrome < 91, node < 16.4",
  "disallow private class fields with in",
  { ts: 2022 }
);
...
export default /** @type {EscompatPlugin} */ (/** @type {unknown} */ (exps));
